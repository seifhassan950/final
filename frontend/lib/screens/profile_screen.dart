...
class _WebProfileState extends State<_WebProfile> {
  ...
  // ✅ NEW: One UI for Followers/Following with tabs + search + follow buttons
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              _stat('$_followersCount', "Followers", onTap: () => _openFollowersFollowing(context, initialIndex: 0)),
              const SizedBox(width: 26),
              _stat('$_followingCount', "Following", onTap: () => _openFollowersFollowing(context, initialIndex: 1)),
              const SizedBox(width: 18),
              _isSelf
                  ? ElevatedButton.icon(
  ...
  Widget _glassGrid() {
    if (_activeTab == ProfileTab.posts) {
      if (_loadingPosts) {
        return const Center(
          child: Padding(padding: EdgeInsets.all(24), child: CircularProgressIndicator(color: Colors.white)),
        );
      }
      if (_postsError != null) {
        return Center(
...
class _MobileProfileState extends State<_MobileProfile> {
  ...
  // ✅ NEW: bottom sheet with tabs + search + follow buttons
  void _openFollowersFollowing(BuildContext context, {required int initialIndex}) {
    final targetUserId = _profileUserId;
    if (targetUserId == null || targetUserId.isEmpty) return;
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  _MStat(value: '$_followersCount', label: "Followers", onTap: () => _openFollowersFollowing(context, initialIndex: 0)),
                  _MStat(value: '$_followingCount', label: "Following", onTap: () => _openFollowersFollowing(context, initialIndex: 1)),
                ],
              ),
              const SizedBox(height: 14),
              SizedBox(
                  ...
                  children: [
                    ...
                    if (showDelete)
                      Positioned(
                        ...
                        child: InkWell(
                          onTap: () async {
                            // keep your delete logic if you have it; omitted here to keep this file focused
                          },
                          borderRadius: BorderRadius.circular(999),
                          child: Container(
                            width: 24,
...
// ──────────────────────────────────────────────────────────
// ✅ NEW SHARED UI: Followers / Following Panel
// ──────────────────────────────────────────────────────────
...
class _FollowersFollowingPanelState extends State<_FollowersFollowingPanel> {
  ...
  // follow status cache: userId -> isFollowing
  final Map<String, bool> _isFollowingCache = {};
  final Set<String> _loadingStatus = {};

  // toggling follow/unfollow per user
  ...
  Future<void> _ensureFollowStatus(String userId) async {
    if (_isFollowingCache.containsKey(userId)) return;
    if (_loadingStatus.contains(userId)) return;
  ...
  Future<void> _toggleFollow(String userId) async {
    ...
    try {
      if (current) {
        await r2vSocial.unfollow(userId);
      } else {
        await r2vSocial.follow(userId);
      }
      if (!mounted) return;
      setState(() => _isFollowingCache[userId] = !current);
    } catch (_) {
      // ignore (you can show snackbar if you want)
    } finally {
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                return ListTile(
                                  ...
                                  leading: _FFAvatar(url: u.avatarUrl),
                                  title: Text(u.username, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                                    ...
                                    child: isLoadingStatus
                                        ...
                                        : OutlinedButton(
                                            ...
                                            style: OutlinedButton.styleFrom(
                                              foregroundColor: Colors.white,
                                              backgroundColor: isFollowing ? Colors.white.withOpacity(0.08) : const Color(0xFF8A4FFF),
...
class _FFTab extends StatelessWidget {
  ...
  const _FFTab({required this.label, required this.active, required this.onTap});
