// home_screen.dart
// ✅ MERGED:
// - Backend summary loading (profile/dashboard/jobs/assets) ✅
// - Mobile glass bottom nav (icons only + animated pill) ✅
// - Mobile top pill only "R2V" ✅
// - Kept stats card style ✅
// - Removed "Quick actions" (web + mobile) ✅
// - Renamed "Use cases" => "Use R2V for" ✅
// - Bottom bar: Home, AI Studio, Scan, Market, Profile ✅
// - Profile tab: View Profile + Settings + Scan tips ✅
...
class _HomeScreenState extends State<HomeScreen> {
  // Mobile bottom-nav:
  // 0 Home, 1 AI, 2 Scan, 3 Market, 4 Profile
  int _selectedTab = 0;
  ...
  // ------------------------------------------------------------------
  // ✅ DATA: Continue / Stats (mutable because backend updates them)
  // ------------------------------------------------------------------
  ...
  void _startUseCaseAutoSwitch() {
    ...
    _useCaseAutoTimer = Timer.periodic(const Duration(seconds: 4), (_) {
      ...
      if (_lastIsWeb) {
        ...
        if (_selectedTab != 0) return; // only Home tab on mobile
      }
  ...
  Widget _buildMobileTopPill(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF0B0D14).withOpacity(0.85),
        borderRadius: BorderRadius.circular(999),
        border: Border.all(color: Colors.white.withOpacity(0.10)),
      ),
      child: Row(
